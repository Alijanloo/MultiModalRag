...
class ElasticsearchDocumentAdaptor(IDocumentIndexRepository):
    ...
    UNIFIED_MAPPING = {
        "mappings": {
            "properties": {
                # Common fields
                "type": {"type": "keyword"},  # "document" or "chunk"
                ...
                "chunk": {
                    "properties": {
                        ...
                        "document_id": {"type": "keyword"}  # Reference to parent document
                    }
                }
            }
        }
    ...
    def __init__(
        ...
        self.UNIFIED_MAPPING["mappings"]["properties"]["chunk"]["properties"]["vector"]["dims"] = vector_dimensions
        ...
        try:
            ...
            if operations:
                ...
                if items:
                    ...
                    doc_responses.append(IndexDocumentResponse(
                        document_id=document_id,
                        success=doc_success,
                        message="Indexed successfully" if doc_success else str(doc_item.get("error", ""))
                    ))
                    
                    if not doc_success:
                        errors.append(f"Document {document_id}: {doc_item.get('error', 'Unknown error')}")
                ...
                for i, item in enumerate(items[1:], 0):
                    ...
                    chunk_responses.append(IndexChunkResponse(
                        chunk_id=chunk_id,
                        success=chunk_success,
                        message="Indexed successfully" if chunk_success else str(chunk_item.get("error", ""))
                    ))
                    
                    if not chunk_success:
                        errors.append(f"Chunk {chunk_id}: {chunk_item.get('error', 'Unknown error')}")
            
            total_indexed = len([r for r in doc_responses if r.success]) + \
                           len([r for r in chunk_responses if r.success])
    ...
    async def search_chunks(self, request: SearchRequest) -> SearchResponse:
        ...
        try:
            ...
            # Build query with type filter for chunks
    ...
    async def search_documents(self, request: SearchRequest) -> SearchResponse:
        ...
        try:
            ...
            # Build text query for documents with type filter
            query = {
                "bool": {
                    "must": [
                        {"term": {"type": "document"}},
                        {
                            "bool": {
                                "should": [
                                    {"match": {"document.name": {"query": request.query, "boost": 2.0}}},
                                    {"match": {"document.origin.filename": {"query": request.query, "boost": 1.5}}}
                                ]
                            }
                        }
                    ]
            ...
            result = await self._es.search(
                ...
                highlight={
                    "fields": {
                        "document.name": {"fragment_size": 150, "number_of_fragments": 1},
                        "document.origin.filename": {"fragment_size": 150, "number_of_fragments": 1}
                    }
                }
            )
        ...
        if request.query:
            text_query = {
                "bool": {
                    ...
                    "should": [
                        {"match": {"chunk.text": {"query": request.query, "boost": 1.0}}},
                        {"match": {"chunk.meta.headings": {"query": request.query, "boost": 1.5}}}
                    ]
                }
            }
