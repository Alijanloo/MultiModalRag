...
from multimodal_rag import DEFAULT_PATH
...
class LoggerFactory:
    ...
    def setup_logging(
        ...
        if console_output:
            ...
            config["loggers"]["multimodal_rag"]["handlers"].append("console")
        ...
        if file_output:
            config["handlers"]["file"] = {
                ...
                "level": log_level,
                "formatter": "standard",
                "filename": str(cls._log_dir / "multimodal_rag.log"),
                "maxBytes": 10485760,
                "backupCount": 5,
            }
            ...
            config["handlers"]["error_file"] = {
                ...
                "level": "ERROR",
                "formatter": "standard",
                "filename": str(cls._log_dir / "multimodal_rag_errors.log"),
                "maxBytes": 10485760,
                "backupCount": 5,
            }

            config["loggers"]["multimodal_rag"]["handlers"].extend(
                ["file", "error_file"]
            )
        ...
        logger = logging.getLogger("multimodal_rag.logging_config")
        logger.info(
    ...
    def get_logger(cls, name: str) -> logging.Logger:
        ...
        if not name.startswith("multimodal_rag"):
            if name == "__main__":
                name = "multimodal_rag.main"
            else:
                name = f"multimodal_rag.{name}"
