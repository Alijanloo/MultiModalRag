...
class GoogleGenAIEmbeddingService(EmbeddingServiceInterface):
    ...
    async def embed_content(self, content: Union[str, List[str]]) -> List[List[float]]:
        ...
        while True:
            ...
            for attempt in range(self._max_retries + 1):
                try:
                    ...
                    logger.info(
                        f"Generated {len(embeddings)} embeddings with API key index {current_key_index}"
                    )
                    return embeddings
                    ...
                    if is_rate_limit_error:
                        ...
                        logger.error(
                            f"Error generating embeddings after {self._max_retries + 1} attempts with API key index {current_key_index}: {str(e)}"
                        )
                        break
                    else:
        ...
        raise RuntimeError(
            f"Failed to generate embeddings after trying all {len(self._api_keys)} available API keys"
        )
    ...
    async def embed_single(self, text: str) -> List[float]:
        ...
        while True:
            ...
            logger.info(
                f"Trying single embedding with API key index {current_key_index}"
            )
            ...
            for attempt in range(self._max_retries + 1):
                try:
                    ...
                    logger.debug(
                        f"Generated embedding for text of length {len(text)} with API key index {current_key_index}"
                    )
                    ...
                    if is_rate_limit_error:
                        ...
                        logger.error(
                            f"Error generating single embedding after {self._max_retries + 1} attempts with API key index {current_key_index}: {str(e)}"
                        )
                        break
                    else:
                        # Try to parse retry delay from error message, fallback to exponential backoff
        ...
        raise RuntimeError(
            f"Failed to generate single embedding after trying all {len(self._api_keys)} available API keys"
        )
