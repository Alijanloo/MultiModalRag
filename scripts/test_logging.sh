...
echo "Multimodal Rag Logging Setup Test"
echo "============================="
...
try:
    from multimodal_rag import get_logger, setup_logging
...
MULTIMODAL_RAG_LOG_LEVEL=DEBUG uv run /tmp/test_debug_logging.py
...
if [ -d "logs" ]; then
    if [ -f "logs/multimodal_rag.log" ]; then
        echo "✓ Main log file created: logs/multimodal_rag.log"
        echo "  Last few entries:"
        tail -3 logs/multimodal_rag.log | sed 's/^/    /'
    else
        echo "? Main log file not found (may not have been created yet)"
    fi

    if [ -f "logs/multimodal_rag_errors.log" ]; then
        echo "✓ Error log file exists: logs/multimodal_rag_errors.log"
    else
...
echo "  MULTIMODAL_RAG_LOG_LEVEL=DEBUG uv run examples/simple_logging_test.py"
